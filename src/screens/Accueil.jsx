    ...
    <div className="animate-screenIn overflow-y-auto no-scrollbar pb-6">
      ...
      <div className="space-y-5 px-5 pt-5">
        ...
        <button onClick={openRoiInfo} className="block w-full text-left tap">
          <span className="tmark"><b>T+</b> / TON RÉSEAU</span>
        ...
        <div className="border border-line p-3.5">
          ...
          <button onClick={openRace} className="mt-3 flex w-full items-center gap-3 text-left tap">
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-0.5 text-[12px] text-fg-muted">Tu sais déjà qui sera sur la ligne. Le réseau commence avant.</div>
            </div>
        ...
        <div className="cadre grid-cols-2">
          <button onClick={openPipeline} className="p-3.5 text-left tap">
            <div className="font-mono text-[9px] font-bold uppercase tracking-label text-fg-faint">Pipeline R.O.I</div>
            ...
            <div className="mt-1.5 text-[11.5px] text-fg-muted">
              {showPipelineValue ? `${pstats.active} relation${pstats.active > 1 ? 's' : ''} active${pstats.active > 1 ? 's' : ''}` : 'Suivi des relations · Pro'}
            </div>
          </button>
          <button onClick={openRunMatch} className="surface-hero p-3.5 text-left tap">
            <div className="font-mono text-[9px] font-bold uppercase tracking-label text-craie/55">RunMatch</div>
            <div className="titre mt-2 text-[15px] text-craie">{topRun ? `Cours avec ${topRun.name.split(' ')[0]}` : 'Ton binôme de run'}</div>
            <div className="mt-1.5 text-[11.5px] text-craie/60">Une sortie à allure de conversation, cette semaine.</div>
          </button>
        </div>

        {/* ---- Le fil ---- */}
        <div>
          <SectionTitle t="T+" action="Écrire" onAction={openComposer}>Le fil</SectionTitle>
          <div className="flex items-center gap-3 border border-line p-3">
            ...
            <button onClick={openComposer} className="input-ligne flex-1 text-left text-[13px] text-fg-faint tap" style={{ padding: '6px 0' }}>
              Une analyse, un retour d’expérience, une opportunité…
            </button>
