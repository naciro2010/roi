...
export default function RouteMapImpl({ route, interactive = false, className = '', lowData = false }) {
  ...
  useEffect(() => {
    ...
    // Tracé : halo craie puis ligne encre, le départ en orange — la ligne, encore.
    L.polyline(latlngs, { color: '#EFEBE2', weight: 7, opacity: 0.9, lineJoin: 'round' }).addTo(map)
    L.polyline(latlngs, { color: '#070707', weight: 4, opacity: 1, lineJoin: 'round' }).addTo(map)

    const start = latlngs[0]
    const end = latlngs[latlngs.length - 1]
    L.circleMarker(start, { radius: 5, color: '#EFEBE2', weight: 2, fillColor: '#FF4400', fillOpacity: 1 }).addTo(map)
    L.circleMarker(end, { radius: 5, color: '#EFEBE2', weight: 2, fillColor: '#070707', fillOpacity: 1 }).addTo(map)
  ...
  return (
    <div
      ref={ref}
      className={className}
      style={lowData ? { background: '#E7E1D4' } : undefined}
