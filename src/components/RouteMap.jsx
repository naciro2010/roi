...
/* Carte d'un parcours de course (Leaflet + tuiles Carto Positron, sobres).
   `interactive=false` → vignette figée (cartes du feed / liste).
   `interactive=true`  → carte manipulable (vue détail). */
export default function RouteMap({ route, interactive = false, className = '' }) {
  ...
  useEffect(() => {
    ...
    // Tracé : halo blanc puis ligne brand pour le contraste.
    L.polyline(latlngs, { color: '#ffffff', weight: 7, opacity: 0.9, lineJoin: 'round' }).addTo(map)
    L.polyline(latlngs, { color: '#4F60A0', weight: 4, opacity: 1, lineJoin: 'round' }).addTo(map)
